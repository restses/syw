...
function setExamList() {
    ...
    $.each(examList, function (index, item) {
        ...
        var html = "<tr id=tr-" + item.id + "><td>" + item.id + "</td><td>" + item.title + "</td><td>" + new Date(item.startTime).toLocaleString() + "</td><td>" + new Date(item.endTime).toLocaleString() + "</td><td><a href='correctStatus.html#" + item.id + "'>状态查询</a></td><td><a href='javascript:correct(" + item.id + ");'>阅卷</a></td></tr>";
...
function correct(paperId) {
    $.get("correct.do?paperId=" + paperId, function (data) {

        if (data.error == 0) {
            alert("操作成功，请稍后到状态查询页查看详情");
        } else if (data.error == 10) {
            window.open("../index.html", "_self");
        } else {
            alert(data.error + " " + data.msg);
