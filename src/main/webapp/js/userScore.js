...
function getPaperMark() {
    var spilt = window.location.hash.indexOf('-');
    paperId = window.location.hash.substring(1, spilt);
    userId = window.location.hash.substring(spilt + 1);

    if (paperId != null) {

        $.get("score.do?paperId=" + paperId + "&userId=" + userId, function (data) {
